	...
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory()
		...
		(
				...
				function() {
					...
					function create_utc__createUTC(input, format, locale,
							strict) {
									...
									: a[DATE] < 1
											...
											: a[HOUR] < 0
													...
													|| (a[HOUR] === 24 && (a[MINUTE] !== 0
															|| a[SECOND] !== 0 || a[MILLISECOND] !== 0)) ? HOUR
													: a[MINUTE] < 0
															|| a[MINUTE] > 59 ? MINUTE
															: a[SECOND] < 0
																	|| a[SECOND] > 59 ? SECOND
																	: a[MILLISECOND] < 0
																			|| a[MILLISECOND] > 999 ? MILLISECOND
																			: -1;
					...
					function configFromISO(config) {
						var i, l, string = config._i, match = from_string__isoRegex
								.exec(string);
					...
					addParseToken('YYYY', function(input, array) {
						array[YEAR] = input.length === 2 ? utils_hooks__hooks
								.parseTwoDigitYear(input) : toInt(input);
					});
					addParseToken('YY', function(input, array) {
						array[YEAR] = utils_hooks__hooks
					...
					function weekOfYear(mom, firstDayOfWeek,
						...
						adjustedMoment = local__createLocal(mom).add(
								daysToDayOfWeek, 'd');
						return {
					...
					var defaultLocaleWeek = {
						...
						doy : 6
					// The week that contains Jan 1st is the first week of the
					// year.
					};
					...
					function dayOfYearFromWeeks(year, week, weekday,
						...
						var week1Jan = 6 + firstDayOfWeek
								- firstDayOfWeekOfYear, janX = createUTCDate(
								year, 0, 1 + week1Jan), d = janX.getUTCDay(), dayOfYear;
						if (d < firstDayOfWeek) {
							d += 7;
						}
					...
					function dayOfYearFromWeekInfo(config) {
						...
						temp = dayOfYearFromWeeks(weekYear, week, weekday, doy,
								dow);
					...
					function configFromStringAndFormat(config) {
						...
						if (getParsingFlags(config).bigHour === true
								&& config._a[HOUR] <= 12 && config._a[HOUR] > 0) {
					...
					function configFromObject(config) {
						...
						config._a = [ i.year, i.month, i.day || i.date, i.hour,
								i.minute, i.second, i.millisecond ];
					...
					function prepareConfig(config) {
						...
						config._locale = config._locale
								|| locale_locales__getLocale(config._l);
					...
					var prototypeMin = deprecate(
							...
							function() {
								var other = local__createLocal.apply(null,
										arguments);
					...
					function Duration(duration) {
						...
						// It is impossible translate months into days without
						// knowing
					...
					function hasAlignedHourOffset(input) {
						input = input ? local__createLocal(input).utcOffset()
								: 0;
					...
					function isDaylightSavingTimeShifted() {
						...
						if (c._a) {
							var other = c._isUTC ? create_utc__createUTC(c._a)
					...
					function add_subtract__addSubtract(mom, duration, isAdding,
						...
						updateOffset = updateOffset == null ? true
								: updateOffset;
						...
						if (days) {
							get_set__set(mom, 'Date', get_set__get(mom, 'Date')
									+ days * isAdding);
						}
						if (months) {
					...
					function moment_calendar__calendar(time, formats) {
						...
						var now = time || local__createLocal(), sod = cloneWithOffset(
								now, this).startOf('day'), diff = this.diff(
								sod, 'days', true), format = diff < -6 ? 'sameElse'
								: diff < -1 ? 'lastWeek' : diff < 0 ? 'lastDay'
										: diff < 1 ? 'sameDay'
												: diff < 2 ? 'nextDay'
														: diff < 7 ? 'nextWeek'
																: 'sameElse';
					...
					function isAfter(input, units) {
						...
						if (units === 'millisecond') {
							input = isMoment(input) ? input
									: local__createLocal(input);
							return +this > +input;
						} else {
							inputMs = isMoment(input) ? +input
									: +local__createLocal(input);
					...
					function isBetween(from, to, units) {
						return this.isAfter(from, units)
					...
					function isSame(input, units) {
						...
						if (units === 'millisecond') {
							...
							return +(this.clone().startOf(units)) <= inputMs
									&& inputMs <= +(this.clone().endOf(units));
						}
					}

					function diff(input, units, asFloat) {
						var that = cloneWithOffset(input, this), zoneDelta = (that
								.utcOffset() - this.utcOffset()) * 6e4, delta, output;
						...
						if (units === 'year' || units === 'month'
							...
							units === 'hour' ? delta / 36e5 : // 1000 * 60 *
																// 60
							units === 'day' ? (delta - zoneDelta) / 864e5 : // 1000
																			// * 60
																			// * 60
																			// *
																			// 24,
																			// negate
																			// dst
							units === 'week' ? (delta - zoneDelta) / 6048e5 : // 1000
																				// * 60
																				// * 60
																				// * 24
																				// * 7,
					...
					function moment_format__toISOString() {
						...
						if (0 < m.year() && m.year() <= 9999) {
							if ('function' === typeof Date.prototype.toISOString) {
								// native implementation is ~50x faster, use it
								// when we can
					...
					function startOf(units) {
						units = normalizeUnits(units);
						// the following switch intentionally omits break
						// keywords
					...
					function weeksInYear(year, dow, doy) {
						return weekOfYear(local__createLocal([ year, 11,
								31 + dow - doy ]), dow, doy).week;
					}
					...
					function getSetISODayOfWeek(input) {
						...
						// week.
						return input == null ? this.day() || 7 : this.day(this
								.day() % 7 ? input : input - 7);
					}
					...
					// explicitly
					// specified which hour he wants. So trying to maintain the
					// same hour (in
					...
					function locale_calendar__calendar(key, mom, now) {
						...
						return typeof output === 'function' ? output.call(mom,
								now) : output;
					}
					...
					function relative__relativeTime(number, withoutSuffix,
							string, isFuture) {
						var output = this._relativeTime[string];
						return (typeof output === 'function') ? output(number,
					...
					function duration_add_subtract__addSubtract(duration,
					...
					function duration_add_subtract__add(input, value) {
						return duration_add_subtract__addSubtract(this, input,
								value, 1);
					}
					...
					function humanize(withSuffix) {
						...
						var output = duration_humanize__relativeTime(this,
								!withSuffix, locale);
