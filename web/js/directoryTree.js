...
(function($) {
    ...
    $.directoryTree = function(containerID, options) {
        ...
        var genTemplateStart = function(dirPaths){
            return '<ul class="root-ul"><li class="'+hasChildrenClassname+'"><span><input type="'+((plugin.settings.select_multiple) ? 'checkbox' : 'radio')+'" class="'+selectElementClassname+' '+rootSelectElementClassname+'" value="/"> Root</span>' + genTemplate(dirPaths,1) + '</li></ul>';
        }
